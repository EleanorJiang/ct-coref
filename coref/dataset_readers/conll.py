...
from coref.util import make_coref_instance

logger = logging.getLogger(__name__)


@DatasetReader.register("my-coref")
class ConllCorefReader(DatasetReader):
    ...
    Returns a `Dataset` where the `Instances` have four fields :
    `text`, a `TextField` containing the full document text,
    `spans`, a `ListField[SpanField]` of inclusive start and end indices for span candidates,
    `metadata`, a `MetadataField` that stores the instance's original text.
    For data with gold cluster labels, we also include the original `clusters` (a list of list of index pairs)
    'span_labels`, a `SequenceLabelField` of cluster ids for every span candidate.
    ...
    def _read(self, file_path: str):
        ...
        # todo: ontonotes的所有meta信息在这里！
        for sentences in ontonotes_reader.dataset_document_iterator(file_path): # every document
            clusters: DefaultDict[int, List[Tuple[int, int]]] = collections.defaultdict(list)
            # defaultdict的作用是在于，当字典里的key不存在但被查找时，返回的不是keyError而是一个默认值
            # dict =defaultdict( factory_function) 这里返回空list=[]
            total_tokens = 0
