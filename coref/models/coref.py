...
class CoreferenceResolver(Model):
    ...
    document.
    These span representations are scored and used to prune away spans that are unlikely
    to occur in a coreference cluster.
    For the remaining spans, the model decides which antecedent span (if any) they are coreferent with.
    The resulting coreference links, after applying transitivity, imply a clustering of the spans in the document.
    ...
    def _coarse_to_fine_pruning(
        ...
        # we need to make `batched_index_select` more general to make this less awkward.
